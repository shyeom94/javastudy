...
public class MainClass {
   ...
   * 2. 생성은 java.sql.DriverManager 클래스가 데이터베이스 접속객체를 생성한다. - Connection 은 저장
   * 3. 형식
   ...
   * 3. 프로퍼티 추가 방법 
   *    1) [Run] - [Run Configurations] - [Arguments] - [VM arguments](인자값을 넘겨줌) 
   *    2) 프로퍼티 앞에 prefix 값으로 -D 를 사용한다. 
   * 4. github.com 에 올릴 수 없는 민감한 정보를 처리하는 용도로 사용한다. 
   * 5. 프로퍼티로 등록한 값은 System.getProperty() 메소드를 이용해서 가져온다. 
   */
  
  
  
  // jdbc 를 위한 준비. 제일 먼저 해야 한다. 
  public static void method1() {
    
    // oracle.jdbc.OracleDriver 클래스 로드하기 (메모리에 올리기)
    
    try {
      Class.forName("oracle.jdbc.OracleDriver"); // String 으로 전달해야 하므로 import 할 수 없다. -> Class.forName();
  ...
  // 프로퍼티 등록 // VM 실행할 때마다 전달이 된다. 
  public static void method4() {
    
    // 사용자 프로퍼티 등록 (jdbc.url, jdbc.user, jdbc.password) [Run] - [Run Configurations] - 프로젝트 찾아서 넣기 
    ...
    String user = System.getenv("jdbc.user");
